...
def fit(epochs, lr,fixed_latent, start_idx=0,name="model",std=0.1,fade_noise=True):
        ...
        trainer = TrainerGDSkip(discriminator, generator, batch_size, device, latent_size)
...
if __name__ == '__main__':
    ...
    generator = GSkip(latent_size, RES).to(device)
    discriminator = DSkip(RES).to(device)
